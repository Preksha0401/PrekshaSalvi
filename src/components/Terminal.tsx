...
const Terminal = () => {
  ...
  return (
    <section id="terminal" className="py-20">
      <div className="container max-w-[740px]" ref={ref}>
        <motion.div
          initial={{ opacity: 0, y: 28 }}
          animate={inView ? { opacity: 1, y: 0 } : {}}
          transition={{ duration: 0.7 }}
        >
          <div className="section-eyebrow mb-2.5">Preksha Shell</div>
          <h2 className="section-title">Try the <em>Terminal</em></h2>

          <div className="glass-panel overflow-hidden" style={{ boxShadow: "0 16px 60px hsla(306,55%,33%,0.16)" }}>
            <div className="flex items-center gap-[7px] px-[18px] py-3" style={{ background: "hsla(260,40%,10%,0.8)" }}>
              <div className="w-3 h-3 rounded-full bg-[#ff5f57]" />
              <div className="w-3 h-3 rounded-full bg-[#febc2e]" />
              <div className="w-3 h-3 rounded-full bg-[#28c840]" />
              <span className="ml-auto font-mono text-[11px] text-muted-foreground opacity-40">preksha@portfolio ~ </span>
            </div>

            <div id="term-output" className="p-[18px] min-h-[270px] max-h-[400px] overflow-y-auto font-mono text-[12.5px] leading-[1.9]">
              {output.map((line, i) => (
                <div key={i} className={
                  line.type === "cmd" ? "text-accent" :
                  line.type === "error" ? "text-[#ff7070]" :
              ...
              <div className="flex items-center gap-[7px] mt-1">
                ...
                <input
                  ...
                  className="flex-1 bg-transparent border-none outline-none text-foreground font-mono text-[12.5px]"
